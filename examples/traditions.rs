...
fn main() {
    ...
    println!("  warmth={:.1} confidence={:.1} courage={:.1}",
        tiphareth.traits.warmth, tiphareth.traits.confidence, tiphareth.traits.courage);
    println!("  breath={:?} growth={:?}", tiphareth.breath, tiphareth.growth);
    println!("  element={:?} polarity={:?} tier={:?}",
    ...
    for t in Trimurti::ALL {
        let p = t.profile();
        println!("  {} — {:?} (intensity {:.2})", p.name, p.breath, p.breath.intensity());
    }
    println!();
    ...
    for p in &brave {
        println!("  {} ({}) — courage={:.2}", p.name, p.tradition, p.traits.courage);
    }
    println!();
    ...
    println!("{total} entities across {n} traditions:", n = traditions.len());
    for t in &traditions {
