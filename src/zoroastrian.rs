...
mod tests {
    ...
    fn rashnu_is_impartial_justice() {
        ...
        assert!(p.traits.precision > 0.9, "Rashnu should have very high precision");
    ...
    fn angra_mainyu_opposes_with_transform() {
        ...
        assert!(p.traits.autonomy > 0.9, "Angra Mainyu should have very high autonomy");
        assert!(p.traits.warmth < 0.2, "Angra Mainyu should have very low warmth");
        assert!(p.traits.empathy < 0.2, "Angra Mainyu should have very low empathy");
    }

    #[test]
    fn verethragna_is_creative_shapeshifter() {
        let p = ZoroastrianBeing::Verethragna.profile();
        assert!(p.traits.courage > 0.9, "Verethragna should have very high courage");
        assert!(p.traits.creativity > 0.8, "Verethragna should have high creativity");
