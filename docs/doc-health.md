...
_Last swept: 2026-07-22 (v2.10.0 — I Ching; 64 hexagrams over the eight trigrams)._

Ground truth at this sweep: **460 archetypes, 29 traditions**; version **2.10.0**;
cyrius pin **6.4.70**; 161 integration tests; 52 benchmarks; `ArchetypeProfile`
...
| `CHANGELOG.md` | 2026-07-22 | ✅ Fresh | Entries through 2.10.0 (I Ching); `[Unreleased]` points at the roadmap (minors → 3.0.0). |
| `CLAUDE.md` | 2026-07-22 | ✅ Fresh | Version 2.10.0, compiler 6.4.70; main.cyr=smoke / tcyr=161 / bcyr=52; all tradition modules incl. `src/tarot.cyr` and `src/iching.cyr` + struct/Result/domain/shadow/xalloc API. |
...
| Doc | Last touched | Status | Notes |
|-----|--------------|--------|-------|
| `docs/development/roadmap.md` | 2026-07-22 | ✅ Fresh | Shipped through 2.10.0 (I Ching); remaining minors v2.11.0 (world-traditions), v2.12.0 (overlays) → v3.0.0 consolidation; Backlog: additive Tarot de Marseille attribution (alongside, not replacing, RWS/Golden Dawn); affinity-graph declined. |
...
| Doc | Last touched | Status | Notes |
|-----|--------------|--------|-------|
| `bench-history.csv` | 2026-07-22 | ✅ Fresh | Rows for 2.4.0–2.10.0 (49/release through 2.8.1; 50 from 2.9.0 with `tarot/all_22`; 52 from 2.10.0 adding the two `iching/*` benches; 2.8.0 skipped bench-recording, restored at 2.8.1). Recorded every release via `scripts/bench-history.sh`; bench is a hard release gate (build + run-to-completion + exit 0). Open item: `affinity/similar_to_5` grows with N² because `similar_to()` fully sorts before trimming — see the roadmap Backlog (see 2.10.0 CHANGELOG). |
